<?php  if (!defined("IS_INITPHP")) exit("Access Denied!");  /* INITPHP Version 1.0 ,Create on 2013-06-09 10:22:49, compiled from D:\xampp\htdocs\test/template/administor/ShowOrder.htm */ ?>
...
<table width="840" border="0" align="center" cellpadding="20">
  ...
  <tr>
    <th align="center" valign="middle" scope="col">Order Id</th>
    <th align="center" valign="middle" scope="col">Create Time</th>
    <th align="center" valign="middle" scope="col">User Name</th>
    <th align="center" valign="middle" scope="col">Branch</th>
    <th align="center" valign="middle" scope="col">Status</th>
    <th align="center" valign="middle" scope="col">Confirm Changes</th>
  </tr>
<?php for ($i=0;$i<$data[1];$i++) { ?>
<?php echo  '<tr>'; ?>
<?php echo  '<form method="post" action="index.php?c=administor&a=changestatuse">'; ?>
<?php echo    '<td align="center" valign="middle">'.$data[0][$i]["id"].'</td>'; ?>
<?php echo    '<td align="center" valign="middle">'.$data[0][$i]["createtime"].'</td>'; ?>
<?php echo    '<td align="center" valign="middle">'.$data[0][$i]["u_id"].'</td>'; ?>
<?php echo    '<td align="center" valign="middle" scope="col">'.$data[0][$i]["branch_id"].'</td>'; ?>
<?php echo    '<td align="center" valign="middle">'; ?>
<?php echo '<select name="Status1">'; ?>
<?php if ($data[0][$i]['statuse']==0) { ?>
<?php echo '<option value="Waiting" selected="selected">Waiting</option>'; ?>
<?php echo '<option value="Serving">Serving</option>'; ?>
<?php echo '<option value="Finished">Finished</option>'; ?>
<?php } elseif  ($data[0][$i]['statuse']==1) { ?>
<?php echo '<option value="Waiting">Waiting</option>'; ?>
<?php echo '<option value="Serving" selected="selected">Serving</option>'; ?>
<?php echo '<option value="Finished">Finished</option>'; ?>
<?php } else { ?>
<?php echo '<option value="Waiting">Waiting</option>'; ?>
<?php echo '<option value="Serving">Serving</option>'; ?>
<?php echo '<option value="Finished" selected="selected">Finished</option>'; ?>
<?php } ?>
<?php echo '</select></td>'; ?>

<?php echo '<input name="id" type="hidden" value='.$data[0][$i]["id"].' />'; ?>
<?php echo	'<td align="center" valign="middle"><input type="submit" value="Confirm" /></td>'; ?>
<?php echo '</form>'; ?>
<?php echo  '</tr>'; ?>
<?php } ?>
</table>
  ...
  <tr>
  	<td bgcolor="#FFFFEE">
    <center>
    	<form method="post" action="index.php?c=administor&a=orderdetail">
        	Please input the order ID for details: <br/> <br/> 
